...
void parseNetwork(const char* acRaw, const char* acId, const char* acName, const char* acY, const char* acZ) 
{
	...
	addNode(&g_System, initNode(new chNode, atoi(acId), afPos, csg_fParseDefaultMass, acName));
}

void parseArc(const char* acRaw, const char* acId0, const char* acId1, const char* acStrength) 
{
	chNode *pN0 = nodeById(&g_System, atoi(acId0));
	chNode *pN1 = nodeById(&g_System, atoi(acId1));

	if (pN0 && pN1) addArc(&g_System, initArc(new chArc, pN0, pN1, (float)atof(acStrength), csg_fParseDefaultSize));
}
...
void parsePartition(const char* acRaw, const char* acValue) 
{
	...
	{
		chNode *pNode = nodeById(&g_System, g_uiParseCount++);
